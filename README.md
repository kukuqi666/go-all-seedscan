...
- 按固定 EVM 派生路径生成地址，扫描多条链的原生币余额
- 实时单行进度显示（已用时间、预计剩余、速度），命中钱包即时输出
...
| `--chains` | `-c` | `all` | 要扫描的链，逗号分隔（如 `eth,bsc`）或 `all` |
...
| `-c all` | 6 条链 | ~10条/s | ~7 小时 |

> 实际速度取决于网络和 RPC 响应，建议优先查目标链以缩短时间。
...
输入助记词模板（? 占位）
  ...
  bip39.IsMnemonicValid() 完整校验
        │
        ▼
  deriveEVMAddress() 派生地址
        │
        ▼
  checkEVMBalance() 并发查询指定链余额（连接池复用）
        │
        ▼
  输出非零余额结果
```
...
- 仅扫描原生币余额，不检查 ERC-20、NFT 或其他资产
- 公开 RPC 可能存在限流或不稳定，结果受网络环境影响
